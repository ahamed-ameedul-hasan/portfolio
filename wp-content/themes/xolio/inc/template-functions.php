 ...
 * @return array
 */
function xolio_body_classes( $classes ) {
...
add_filter( 'body_class', 'xolio_body_classes' );

/**
 * Get tags.
 */
function xolio_get_tag() {
    $html = '';
    if ( has_tag() ) {
        $html .= '<div class="tg-post-tag"><h5 class="tag-title">' . esc_html__( 'Post Tags :', 'xolio' ) . '</h5>';
...
/** img alt-text **/
function xolio_img_alt_text( $img_er_id = null ) {
    $image_id = $img_er_id;
    ...
    if ( !empty( $image_id ) ) {
        ...
        $alt_text = esc_html__( 'Image Alt Text', 'xolio' );
    }

    return $alt_text;
}

// xolio_offer_sidebar_func
function xolio_offer_sidebar_func() {
    if ( is_active_sidebar( 'offer-sidebar' ) ) {

        dynamic_sidebar( 'offer-sidebar' );
    }
}
add_action( 'xolio_offer_sidebar', 'xolio_offer_sidebar_func', 20 );

// xolio_service_sidebar
function xolio_service_sidebar_func() {
    if ( is_active_sidebar( 'services-sidebar' ) ) {

        dynamic_sidebar( 'services-sidebar' );
    }
}
add_action( 'xolio_service_sidebar', 'xolio_service_sidebar_func', 20 );

// xolio_portfolio_sidebar
function xolio_portfolio_sidebar_func() {
    if ( is_active_sidebar( 'portfolio-sidebar' ) ) {

        dynamic_sidebar( 'portfolio-sidebar' );
    }
}
add_action( 'xolio_portfolio_sidebar', 'xolio_portfolio_sidebar_func', 20 );

// xolio_faq_sidebar
function xolio_faq_sidebar_func() {
    if ( is_active_sidebar( 'faq-sidebar' ) ) {

        dynamic_sidebar( 'faq-sidebar' );
    }
}
add_action( 'xolio_faq_sidebar', 'xolio_faq_sidebar_func', 20 );